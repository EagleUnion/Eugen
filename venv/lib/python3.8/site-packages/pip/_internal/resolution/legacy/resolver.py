...
class Resolver(BaseResolver):
    ...
    def _is_upgrade_allowed(self, req):
        ...
        if self.upgrade_strategy is "to-satisfy-only":
            return False
        elif self.upgrade_strategy is "eager":
            return True
        else:
            assert self.upgrade_strategy is "only-if-needed"
    ...
    def _check_skip_installed(self, req_to_install):
        ...
        if not self._is_upgrade_allowed(req_to_install):
            if self.upgrade_strategy is "only-if-needed":
    ...
    def _get_abstract_dist_for(self, req):
        ...
        if req.satisfied_by:
            should_modify = (
                self.upgrade_strategy is not "to-satisfy-only" or
                self.force_reinstall or
                self.ignore_installed or
                req.link.scheme is 'file'
            )
            if should_modify:
