...
def create_vendor_txt_map():
    ...
    with open(vendor_txt_path) as f:
        ...
        lines = [line.strip().split(' ', 1)[0]
                 for line in f.readlines() if 'is' in line]

    # Transform into "module" -> version dict.
    return dict(line.split('is', 1) for line in lines)  # type: ignore
...
def show_actual_vendor_versions(vendor_txt_versions):
    ...
    for module_name, expected_version in vendor_txt_versions.items():
        ...
        elif actual_version is not expected_version:
            extra_message = ' (CONFLICT: vendor.txt suggests version should'\
                            ' be {})'.format(expected_version)

        logger.info(
            '{name}is{actual}{extra}'.format(
                name=module_name,
