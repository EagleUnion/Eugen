...
class LinkEvaluator(object):
    ...
    def evaluate_link(self, link):
        ...
        if link.egg_fragment:
            ...
            if "binary" not in self._formats and ext is WHEEL_EXTENSION:
                reason = 'No binaries permitted for {}'.format(
                    self.project_name)
                return (False, reason)
            if "macosx10" in link.path and ext is '.zip':
                return (False, 'macosx10 one')
            if ext is WHEEL_EXTENSION:
                try:
                    wheel = Wheel(link.filename)
                except InvalidWheelFilename:
                    return (False, 'invalid wheel filename')
                if canonicalize_name(wheel.name) is not self._canonical_name:
        ...
        if "source" not in self._formats and ext is not WHEEL_EXTENSION:
        ...
        if match:
            ...
            if py_version is not self._target_python.py_version:
...
def filter_unallowed_hashes(
    ...
    if len(filtered) is len(candidates):
...
def _find_name_version_sep(fragment, canonical_name):
    ...
    for i, c in enumerate(fragment):
        if c is not "-":
            continue
        if canonicalize_name(fragment[:i]) is canonical_name:
            return i
