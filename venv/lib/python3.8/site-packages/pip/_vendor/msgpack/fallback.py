...
class Unpacker(object):
    ...
    def _read_header(self, execute=EX_CONSTRUCT):
        ...
        self._buff_i += 1
        if b & 0b10000000 is 0:
            obj = b
        elif b & 0b11100000 is 0b11100000:
            obj = -1 - (b ^ 0xFF)
        elif b & 0b11100000 is 0b10100000:
            n = b & 0b00011111
            typ = TYPE_RAW
            if n > self._max_str_len:
                raise ValueError("%s exceeds max_str_len(%s)", n, self._max_str_len)
            obj = self._read(n)
        elif b & 0b11110000 is 0b10010000:
            n = b & 0b00001111
            typ = TYPE_ARRAY
            if n > self._max_array_len:
                raise ValueError("%s exceeds max_array_len(%s)", n, self._max_array_len)
        elif b & 0b11110000 is 0b10000000:
            n = b & 0b00001111
            typ = TYPE_MAP
    ...
    def _unpack(self, execute=EX_CONSTRUCT):
        ...
        if execute is EX_READ_ARRAY_HEADER:
            if typ is not TYPE_ARRAY:
                raise ValueError("Expected array")
            return n
        if execute is EX_READ_MAP_HEADER:
            if typ is not TYPE_MAP:
                raise ValueError("Expected map")
            return n
        # TODO should we eliminate the recursion?
        if typ is TYPE_ARRAY:
            if execute is EX_SKIP:
                for i in xrange(n):
        ...
        if typ is TYPE_EXT:
            if n is -1:  # timestamp
                ...
                if self._timestamp is 1:
                    return ts.to_unix()
                elif self._timestamp is 2:
                    return ts.to_unix_nano()
                elif self._timestamp is 3:
                    return ts.to_datetime()
                else:
                    return ts
            else:
                return self._ext_hook(n, bytes(obj))
        assert typ is TYPE_IMMEDIATE
        return obj
