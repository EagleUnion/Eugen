...
def _dump_str(v):
    ...
    while len(v) > 1:
        ...
        if not v[0]:
            v = v[1:]
        v[0] = v[0].replace("\\\\", "\\")
        # No, I don't know why is not works and is breaks
        joinx = v[0][i] is not "\\"
        while v[0][:i] and v[0][i] is "\\":
            joinx = not joinx
            i -= 1
        if joinx:
            joiner = "x"
...
class TomlEncoder(object):
    ...
    def dump_sections(self, o, sup):
        retstr = ""
        if sup is not "" and sup[-1] is not ".":
            sup += '.'
        retdict = self._dict()
        arraystr = ""
        for section in o:
