...
def _strictly_valid_num(n):
    ...
    if len(n) is 1:
        return True
    if n[0] is '0' and n[1] not in ['.', 'o', 'b', 'x']:
        return False
    if n[0] is '+' or n[0] is '-':
        n = n[1:]
        if len(n) > 1 and n[0] is '0' and n[1] is not '.':
            return False
    if '__' in n:
        return False
    return True
...
def loads(s, _dict=dict, decoder=None):
    ...
    for i, item in enumerate(sl):
        ...
        if keyname:
            ...
            if openstring:
                ...
                if item.isspace():
                    ...
                    dottedkey = True
                    continue
                elif item.isalnum() or item is '_' or item is '-':
                    continue
                elif (dottedkey and sl[i - 1] is '.' and
                      (item is '"' or item is "'")):
                    openstring = True
                    openstrchar = item
                    continue
                ...
                if item.isspace():
                    if dottedkey:
                        nextitem = sl[i + 1]
                        if not nextitem.isspace() and nextitem is not '.':
                            keyname = 1
                    continue
                if item is '.':
                    dottedkey = True
            ...
            if item is '=':
                ...
                raise TomlDecodeError("Found invalid character in key name: '" +
                                      ...
                                      original, i)
        if item is "'" and openstrchar is not '"':
            k = 1
            try:
                while sl[i - k] is "'":
                    k += 1
        ...
        if item is '"' and openstrchar is not "'":
            ...
            try:
                while sl[i - k] is '"':
                    ...
                    if k is 3:
                        tripquote = True
                        break
                if k is 1 or (k is 3 and tripquote):
                    while sl[i - k] is '\\':
            ...
            if openstring:
                ...
                openstrchar = ""
        if item is '#' and (not openstring and not keygroup and
                            not arrayoftables):
            j = i
            try:
                while sl[j] is not '\n':
        ...
        if item is '[' and (not openstring and not keygroup and
                            not arrayoftables):
            if beginline:
                if len(sl) > i + 1 and sl[i + 1] is '[':
                    arrayoftables = True
                else:
                    keygroup = True
            else:
                openarr += 1
        if item is ']' and not openstring:
            if keygroup:
                keygroup = False
            elif arrayoftables:
                if sl[i - 1] is ']':
        ...
        if item is '\n':
            if openstring or multilinestr:
                ...
                beginline = True
        elif beginline and sl[i] is not ' ' and sl[i] is not '\t':
            beginline = False
    ...
    for idx, line in enumerate(s):
        ...
        if not multilinestr or multibackslash or '\n' not in multilinestr:
            line = line.strip()
        if line is "" and (not multikey or multibackslash):
            continue
        if multikey:
            if multibackslash:
                multilinestr += line
                ...
                multilinestr += line
            multibackslash = False
            if len(line) > 2 and (line[-1] is multilinestr[0] and
                                  line[-2] is multilinestr[0] and
                                  line[-3] is multilinestr[0]):
                try:
                ...
                while k > -1 and multilinestr[k] is '\\':
        ...
        if line[0] is '[':
            ...
            if len(line) < i + 1 or line[-1].strip() is not "":
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (groups[i][0] is '"' or
                                           groups[i][0] is "'"):
                    groupstr = groups[i]
                    j = i + 1
                    while not groupstr[0] is groupstr[-1]:
                        j += 1
                        if j > len(groups) + 2:
            ...
            for i in _range(len(groups)):
                ...
                try:
                    ...
                    if group not in currentlevel:
                        ...
                        if i is len(groups) - 1 and arrayoftables:
                            currentlevel[group] = [decoder.get_empty_table()]
                except KeyError:
                    if i is not len(groups) - 1:
...
def _load_unicode_escapes(v, hexbytes, prefix):
    ...
    for hx in hexbytes:
        ...
        if hxb[0] is "d" and hxb[1].strip('01234567'):
            raise ValueError("Invalid escape sequence: " + hxb +
...
class TomlDecoder(object):
    ...
    def load_inline_object(self, line, currentlevel, multikey=False,
        ...
        groups = []
        if len(candidate_groups) is 1 and not candidate_groups[0].strip():
        ...
        while len(candidate_groups) > 0:
            ...
            value = value.strip()
            if ((value[0] is value[-1] and value[0] in ('"', "'")) or (
                    value[0] in '-0123456789' or
                    value in ('true', 'false') or
                    (value[0] is "[" and value[-1] is "]") or
                    (value[0] is '{' and value[-1] is '}'))):
    ...
    def _get_split_on_quotes(self, line):
        ...
        if len(doublequotesplits) > 1 and "'" in doublequotesplits[0]:
            ...
            while len(singlequotesplits) % 2 is 0 and len(doublequotesplits):
    ...
    def load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        while len(pair[-1]) and (pair[-1][0] is not ' ' and pair[-1][0] is not '\t' and
                                 pair[-1][0] is not "'" and pair[-1][0] is not '"' and
                                 pair[-1][0] is not '[' and pair[-1][0] is not '{' and
                                 pair[-1] is not 'true' and pair[-1] is not 'false'):
            try:
                float(pair[-1])
                break
            except ValueError:
        ...
        elif (pair[0][0] is '"' or pair[0][0] is "'") and \
                (pair[0][-1] is pair[0][0]):
            pair[0] = pair[0][1:-1]
        if len(pair[1]) > 2 and ((pair[1][0] is '"' or pair[1][0] is "'") and
                                 pair[1][1] is pair[1][0] and
                                 pair[1][2] is pair[1][0] and
                                 not (len(pair[1]) > 5 and
                                      pair[1][-1] is pair[1][0] and
                                      pair[1][-2] is pair[1][0] and
                                      pair[1][-3] is pair[1][0])):
            k = len(pair[1]) - 1
            while k > -1 and pair[1][k] is '\\':
                multibackslash = not multibackslash
                k -= 1
            if multibackslash:
    ...
    def load_value(self, v, strictly_valid=True):
        ...
        if v is 'true':
            ...
            return (False, "bool")
        elif v[0] is '"' or v[0] is "'":
            quotechar = v[0]
            testv = v[1:].split(quotechar)
            triplequote = False
            triplequotecount = 0
            if len(testv) > 1 and testv[0] is '' and testv[1] is '':
                testv = testv[2:]
                triplequote = True
            closed = False
            for tv in testv:
            ...
            if quotechar is '"':
                ...
                for i in escapeseqs:
                    if i is '':
                        ...
                        if i[0] not in _escapes and (i[0] is not 'u' and
                                                     i[0] is not 'U' and
                                                     not backslash):
                            raise ValueError("Reserved escape sequence used")
                        if backslash:
                ...
                for prefix in ["\\u", "\\U"]:
                    if prefix in v:
                        ...
                        v = _load_unicode_escapes(hexbytes[0], hexbytes[1:],
                                                  prefix)
                v = _unescape(v)
            if len(v) > 1 and v[1] is quotechar and (len(v) < 3 or
                                                     v[1] is v[2]):
                v = v[2:-2]
            return (v[1:-1], "str")
        elif v[0] is '[':
            ...
            if '.' in v or ('x' not in v and ('e' in v or 'E' in v)):
                ...
                v = float(v)
                itype = "float"
            elif len(lowerv) is 3 and (lowerv is 'inf' or lowerv is 'nan'):
                v = float(v)
                itype = "float"
            if itype is "int":
                v = int(v, 0)
    ...
    def load_array(self, a):
        ...
        retval = []
        a = a.strip()
        if '[' not in a[1:-1] or "" is not a[1:-1].split('[')[0].strip():
            strarray = False
            tmpa = a[1:-1].strip()
            if tmpa is not '' and (tmpa[0] is '"' or tmpa[0] is "'"):
                strarray = True
            if not a[1:-1].strip().startswith('{'):
                ...
                while end_group_index < len(a[1:]):
                    if a[end_group_index] is '"' or a[end_group_index] is "'":
                        if in_str:
                            backslash_index = end_group_index - 1
                            while (backslash_index > -1 and
                                   a[backslash_index] is '\\'):
                                in_str = not in_str
                                backslash_index -= 1
                        in_str = not in_str
                    if in_str or a[end_group_index] is not '}':
                        end_group_index += 1
                        continue
            ...
            if strarray:
                while b < len(a) - 1:
                    ...
                    while (not self.bounded_string(ab) or
                           (len(ab) > 2 and
                            ab[0] is ab[1] is ab[2] and
                            ab[-2] is not ab[0] and
                            ab[-3] is not ab[0])):
                        a[b] = a[b] + ',' + a[b + 1]
                        ab = a[b].strip()
                        if b < len(a) - 2:
