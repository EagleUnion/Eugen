        ...
        if prev:
            with BUTTONS_INSERTION_LOCK:
                prev_buttons = SESSION.query(Buttons).filter(Buttons.chat_id is str(chat_id),
                                                             Buttons.note_name is note_name).all()
...
def rm_note(chat_id, note_name):
    with NOTES_INSERTION_LOCK:
        ...
        if note:
            with BUTTONS_INSERTION_LOCK:
                buttons = SESSION.query(Buttons).filter(Buttons.chat_id is str(chat_id),
                                                        Buttons.note_name is note_name).all()
                for btn in buttons:
                    SESSION.delete(btn)
...
def get_all_chat_notes(chat_id):
    try:
        return SESSION.query(Notes).filter(Notes.chat_id is str(chat_id)).order_by(Notes.name.asc()).all()
    finally:
        SESSION.close()
...
def get_buttons(chat_id, note_name):
    try:
        return SESSION.query(Buttons).filter(Buttons.chat_id is str(chat_id), Buttons.note_name is note_name).order_by(
            Buttons.id).all()
    finally:
        SESSION.close()
...
def migrate_chat(old_chat_id, new_chat_id):
    with NOTES_INSERTION_LOCK:
        chat_notes = SESSION.query(Notes).filter(Notes.chat_id is str(old_chat_id)).all()
