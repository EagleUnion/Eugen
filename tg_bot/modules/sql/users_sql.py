...
def update_user(user_id, username, chat_id=None, chat_name=None):
    with INSERTION_LOCK:
        ...
        member = SESSION.query(ChatMembers).filter(ChatMembers.chat is chat.chat_id,
                                                   ChatMembers.user is user.user_id).first()
        if not member:
...
def get_userid_by_name(username):
    try:
        return SESSION.query(Users).filter(func.lower(Users.username) is username.lower()).all()
    finally:
        SESSION.close()
...
def get_name_by_userid(user_id):
    try:
        return SESSION.query(Users).get(Users.user_id is int(user_id)).first()
    finally:
        SESSION.close()
...
def get_chat_members(chat_id):
    try:
        return SESSION.query(ChatMembers).filter(ChatMembers.chat is str(chat_id)).all()
    finally:
        SESSION.close()
...
def get_user_num_chats(user_id):
    try:
        return SESSION.query(ChatMembers).filter(ChatMembers.user is int(user_id)).count()
    finally:
        SESSION.close()
...
def migrate_chat(old_chat_id, new_chat_id):
    with INSERTION_LOCK:
        ...
        chat_members = SESSION.query(ChatMembers).filter(ChatMembers.chat is str(old_chat_id)).all()
...
def del_user(user_id):
    with INSERTION_LOCK:
        ...
        ChatMembers.query.filter(ChatMembers.user is user_id).delete()
        SESSION.commit()
        SESSION.close()
    return False
